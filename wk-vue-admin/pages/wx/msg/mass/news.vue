<template>
  <div class="platform-content">
    <h4 class="platform-content-title">
      <span>{{ $t(`wx.msg.mass.news`) }}
        <el-button type="text" icon="fa fa-arrow-circle-left" @click="goBack"> {{ $t(`system.commons.button.goback`) }}</el-button>
      </span>
      ...
      <div class="platform-content-list">
        <div class="platform-content-list-table">
          ...
          >
            ...
            <el-table-column :label="$t(`wx.msg.mass.news.form.author`)" header-align="left" prop="author" :show-overflow-tooltip="true" width="120" />
    ...
    <el-dialog
      :title="$t(`wx.msg.mass.news.create`)"
    ...
    >
      <el-form
        ...
        label-width="160px"
      >
        <el-form-item prop="title" :label="$t(`wx.msg.mass.news.form.wxname`)">
          {{ formData.wxname }}
        </el-form-item>

        <el-form-item prop="title" :label="$t(`wx.msg.mass.news.form.title`)">
          <el-input
            v-model="formData.title"
            maxlength="255"
            :placeholder="$t(`wx.msg.mass.news.form.title`)"
            auto-complete="off"
            tabindex="1"
            type="text"
          />
        </el-form-item>
        <el-form-item prop="author" :label="$t(`wx.msg.mass.news.form.author`)">
          <el-input
            v-model="formData.author"
            maxlength="50"
            :placeholder="$t(`wx.msg.mass.news.form.author`)"
            auto-complete="off"
            tabindex="2"
            type="text"
          />
        ...
        <el-form-item prop="picurl" :label="$t(`wx.msg.mass.news.form.picurl`)">
          <el-upload
        ...
        <el-form-item prop="digest" :label="$t(`wx.msg.mass.news.form.digest`)">
          <el-input v-model="formData.digest" type="textarea" />
        </el-form-item>

        <el-form-item prop="content_source_url" :label="$t(`wx.msg.mass.news.form.content_source_url`)">
          <el-checkbox v-model="checkedSourceUrl">{{ $t(`wx.msg.mass.news.form.content_source_url`) }}</el-checkbox>
          <el-input
            v-if="checkedSourceUrl"
            v-model="formData.url"
            maxlength="50"
            :placeholder="$t(`wx.msg.mass.news.form.content_source_url`)"
            auto-complete="off"
            tabindex="2"
            type="text"
          />
        ...
        <el-form-item prop="show_cover_pic" :label="$t(`wx.msg.mass.news.form.show_cover_pic`)">
          <el-checkbox v-model="formData.show_cover_pic" true-label="1" false-label="0">{{ $t(`wx.msg.mass.news.form.show_cover_pic`) }}</el-checkbox>
          <el-alert
            v-if="formData.show_cover_pic=='1'"
            :title="$t(`wx.msg.mass.news.form.show_cover_pic.tip1`)"
            type="info"
            :close-text="$t(`wx.msg.mass.news.form.show_cover_pic.tip2`)"
          />
        </el-form-item>

        <el-form-item prop="need_open_comment" :label="$t(`wx.msg.mass.news.form.need_open_comment`)">
          <el-checkbox v-model="formData.need_open_comment" true-label="1" false-label="0">{{ $t(`wx.msg.mass.news.form.need_open_comment`) }}</el-checkbox>
        </el-form-item>

        <el-form-item prop="only_fans_can_comment" :label="$t(`wx.msg.mass.news.form.only_fans_can_comment`)">
          <el-checkbox v-model="formData.only_fans_can_comment" true-label="1" false-label="0">{{ $t(`wx.msg.mass.news.form.only_fans_can_comment.tip`) }}</el-checkbox>
        </el-form-item>

        <el-form-item prop="content" :label="$t(`wx.msg.mass.news.form.content`)">
          <div>
            <QuillEditor
              ref="myQuillEditor"
              style="height:320px;"
        ...
        <el-form-item style="padding-top:75px;">
          <el-alert
            ...
            :title="$t(`wx.msg.mass.news.form.content.tip2`)"
            type="info"
            :close-text="$t(`wx.msg.mass.news.form.show_cover_pic.tip2`)"
          />
          <el-alert
            style="margin-top: 5px;"
            :title="$t(`wx.msg.mass.news.form.content.tip1`)"
            type="info"
            :close-text="$t(`wx.msg.mass.news.form.show_cover_pic.tip2`)"
          />
        </el-form-item>
    ...
    >
      ...
      <el-row>
        <span style="font-size:12px;">{{ $t(`wx.msg.mass.news.form.author`) }}： {{ formData.author }}</span>
      </el-row>
...
export default {
  ...
  computed: {
    ...
    formRules() {
      const formRules = {
        title: [
          { required: true, message: this.$t(`wx.msg.mass.news.form.title`), trigger: 'blur' }
        ],
        picurl: [
          { required: true, message: this.$t(`wx.msg.mass.news.form.picurl`), trigger: 'blur' }
        ]
      }
      return formRules
    }
  ...
  methods: {
    ...
    beforeImageUpload(file) {
      ...
      if (!isLt2M) {
        this.$message.error(this.$t(`wx.msg.mass.news.form.picurl.tip2`, { size: '64KB' }))
        return false
      }
      return true
    },
    ...
    deleteRow(row) {
      this.$confirm(
        this.$t(`wx.msg.mass.news.delete`),
    ...
    goBack() {
      this.$router.push('/platform/wx/msg/mass')
    }
  }
}
</script>
