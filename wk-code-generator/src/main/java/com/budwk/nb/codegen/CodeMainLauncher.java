...
public class CodeMainLauncher {
    ...
    public void init() throws Exception {
        ...
        for (Map.Entry<String, TableDescriptor> entry : tables.entrySet()) {
            ...
            // 生成服务接口类
            File fileService = new File(baseProjectName + "/wk-nb-web/" + outputDir, packagePath + "/" + table.getModelName() + "/services/" + table.getServiceClassName() + ".java");
            log.debug("generate service : " + fileService.getName());
            generator.generate(basePackageName, "template/service.vm", fileService, force);
            // 生成服务实现类
            File fileServiceImpl = new File(baseProjectName + "/wk-nb-web/" + outputDir, packagePath + "/" + table.getModelName() + "/services/impl/" + table.getServiceClassName() + "Impl.java");
            ...
            // 生成控制类
            File fileController = new File(baseProjectName + "/wk-nb-web/" + outputDir, packagePath + "/web/controllers/platform/" + table.getModelName() + "/" + table.getControllerClassName() + ".java");
