...
public class WxService {
    ...
    public synchronized WxApi2 getWxApi2Mina(String id) {
        WxApi2Impl wxApi2 = Globals.WxMap.getAs(id, WxApi2Impl.class);
        if (wxApi2 == null) {
            Wx_mina appInfo = wxMinaService.fetch(id);
            JedisAgenAccessTokenStore redisAccessTokenStore = new JedisAgenAccessTokenStore(RedisConstant.REDIS_KEY_WX_TOKEN + id, jedisAgent);
            wxApi2 = new WxApi2Impl();
            wxApi2.setAppid(appInfo.getAppid());
            wxApi2.setAppsecret(appInfo.getAppsecret());
            wxApi2.setAccessTokenStore(redisAccessTokenStore);
            Globals.WxMap.put(id, wxApi2);
        }
        return wxApi2;
    }
}
